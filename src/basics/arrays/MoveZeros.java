...
public class MoveZeros {
        ...
        private int[] moveZeroes(int[] nums) {
            ...
            int endZero = -1, startZero = -1;
            for(int x=0; x<nums.length; x++) {

                if(nums[x] == 0 && (x+1) < nums.length && nums[x+1] != 0) {
                    // 1st most element is zero
                    if(startZero < 0){
                        endZero = x;
                    }
                    swapElements(nums, endZero, x+1);
                    startZero=x+1;
                    endZero++;
                }
                else if (nums[x] == 0 && (x+1) < nums.length && nums[x+1] == 0){
                    endZero = x;
                    startZero = x+1;
                    if((x-1)>=0 && nums[x-1] == 0){
                        endZero = x-1;
        ...
        private void swapElements(int[] nums, int target, int source){
            nums[target] = nums[source];
            nums[source] = 0;
        }

        public static void main(String[] args){
            int[] nums = {-959151711,623836953,209446690,-1950418142,1339915067,-733626417,481171539,-2125997010,
                    -1225423476,1462109565,147434687,-1800073781,-1431212205,-450443973,50097298,753533734,
                    -747189404,-2070885638,0,-1484353894,-340296594,-2133744570,619639811,-1626162038,669689561,
                    0,112220218,502447212,-787793179,0,-726846372,-1611013491,204107194,1605165582,-566891128,
